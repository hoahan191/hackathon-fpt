...
=======
| REQ-003 | Entries are returned by timestamp descending. If timestamps match, entries recorded later are returned first, and the order is stable across repeated queries. | This keeps "most recent first" deterministic even when the supplied clock is coarse. |
| REQ-004 | A metadata value is sensitive when its key contains `password`, `token`, `secret`, `apiKey`, `authorization`, `creditCard`, or `phone`, case-insensitively. Redaction is irreversible and stores the placeholder `[REDACTED]`. | Key-based rules are simple to explain, test, and apply before storage; irreversible redaction avoids keeping sensitive data in memory. |
| REQ-005 | Time ranges are inclusive at both the start and the end. | Inclusive boundaries match how people usually ask "between 12:00 and 13:00" and avoid surprising dropped boundary events. |
| REQ-006 | No matches always returns an empty array; callers cannot distinguish "actor exists but did nothing" from "actor does not exist". | The activity log stores events, not actor accounts, so it has no reliable source for actor existence. |
>>>>>>> Stashed changes
